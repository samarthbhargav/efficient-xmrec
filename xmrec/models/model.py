...
def init_market_embedding(model, config):
    ...
    if config.get("market_aware", False):
        ...
        if config.get("embedding_market") is None:
            model.embedding_market = torch.nn.Embedding(num_embeddings=model.num_markets,
...
def transform_market_aware(model, user_embedding, item_embedding, market_indices, mult="item"):
    assert mult in {"all", "user", "item"}
    if model.market_aware:
        ...
        if mult == "all":
            ...
            user_embedding = torch.mul(user_embedding, market_embedding)
        elif mult == "user":
            user_embedding = torch.mul(user_embedding, market_embedding)
        elif mult == "item":
            item_embedding = torch.mul(item_embedding, market_embedding)
...
class GMF(torch.nn.Module):
    ...
    def forward(self, user_indices, item_indices, market_indices=None):
        ...
        if self.trainable_item:
            ...
            item_embedding = self.embedding_item[item_indices]
        user_embedding, item_embedding = transform_market_aware(self, user_embedding, item_embedding, market_indices)
...
class NeuMF(torch.nn.Module):
    ...
    def forward(self, user_indices, item_indices, market_indices=None):
        ...
        user_embedding_mlp, item_embedding_mlp = transform_market_aware(self,
                                                                        user_embedding_mlp,
                                                                        item_embedding_mlp,
                                                                        market_indices)
